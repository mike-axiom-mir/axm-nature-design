...
const CONTEXTS := ["ground_oblique", "crown_underside"]
...
func build_full_mesh(payload: Dictionary) -> Dictionary:
    ...
    for triangle_value in triangles:
        ...
        # Reverse one edge so source front-face sense is preserved.
...
func camera_spec(context: String) -> Dictionary:
    if context == "crown_underside":
        return {
            "position": Vector3(2.8, 1.65, 2.8),
            "target": Vector3(0.0, 3.55, 0.0),
            "fov": 43.0,
        }
    return {
        "position": Vector3(3.9, 2.35, 4.4),
...
func _initialize() -> void:
    ...
    for sample_id_value in SAMPLE_IDS:
        ...
        var front_path: String = GENERATED_DIR + "/frame_" + sample_id + "ms_front_mesh.json"
        var candidate_path: String = GENERATED_DIR + "/frame_" + sample_id + "ms_leaf_backface_mesh.json"
        var front := read_json(front_path)
        var candidate := read_json(candidate_path)
    ...
    for context_value in CONTEXTS:
        ...
        for sample_id_value in SAMPLE_IDS:
            ...
            var recovery := pixel_difference(front_back, candidate_back)
            candidate_recovery[prefix] = recovery
    ...
    var underside_neutral_key := "crown_underside/0000"
    if int((baseline_culling_loss[underside_neutral_key] as Dictionary)["changed_pixels"]) <= 0:
        fail("crown underside control did not expose the single-sided leaf culling gap")
        return
    ...
    for context_value in CONTEXTS:
        ...
        var returned := pixel_difference(
            images[context + "/0000/candidate/back"] as Image,
            images[context + "/0500/candidate/back"] as Image
        )
        ...
        var moved := pixel_difference(
            images[context + "/0000/candidate/back"] as Image,
            images[context + "/0250/candidate/back"] as Image
        )
        ...
        var mirrored := pixel_difference(
            images[context + "/0125/candidate/back"] as Image,
            images[context + "/0375/candidate/back"] as Image
        )
