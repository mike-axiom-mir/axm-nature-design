...
def main() -> int:
    out = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "evidence" / "compact-east-tree-neutral-001"
    out.mkdir(parents=True, exist_ok=True)
    source = load_source(SOURCE)
    mesh = build_mesh(source)
    report = evaluate(source)
    if report["status"] != "PASS_COMPACT_SOURCE_ENVELOPE":
        raise SystemExit("compact east tree evidence failed")

    _write_json(out / "source.json", source)
    _write_json(out / "mesh.json", mesh)
    _write_json(out / "evidence.json", report)
    _write_obj(mesh, out / "compact-east-tree-neutral-001.obj", source["study_id"])
