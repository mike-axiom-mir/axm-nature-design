...
class RearTreeDeformationReadinessTests(unittest.TestCase):
    ...
    def test_current_source_holds_only_on_missing_branch_root_flex_zone(self):
        ...
        self.assertFalse(report["checks"]["all_primary_branch_roots_have_exact_declared_flex_zone"])
    ...
    def test_exact_current_minimum_support_is_north_top_and_positive(self):
        ...
        self.assertIsNone(north_top["exact_root_flex_zone_id"])

    def test_declaring_a_matching_unproven_zone_closes_only_the_coverage_hold(self):
        candidate = copy.deepcopy(self.source)
        candidate["flex_zones"].append(
            {
                "id": "north-top-branch-flex-candidate",
